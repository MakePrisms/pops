...
const nextConfig = {
  ...
  // `serverExternalPackages` alone does not stop Next 15 from inlining the glue
  // during the build-time page-data pass, so also force the package to a runtime
  // CommonJS `require` for the server build. Combined with the route's lazy
  // `import()`, the module is neither evaluated at build nor bundled: it loads
  // from node_modules at request time with `__dirname` intact.
